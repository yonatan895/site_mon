...
class TestEndToEndPipeline:
    ...
    def test_poller_spool_sender_flow(self, tmp_spool_dir: str) -> None:
    ...
    @pytest.mark.integration
    def test_retry_and_dead_letter_flow(self, tmp_spool_dir: str) -> None:
        spool_mgr = SpoolManager(spool_dir=tmp_spool_dir)
        ...
        assert (
            ...
            or stats["pending_count"] + stats["processing_count"] == 0
        )

    @pytest.mark.integration
    def test_hec_ndjson_send_with_mock(self, tmp_spool_dir: str) -> None:
        spool_mgr = SpoolManager(spool_dir=tmp_spool_dir)
        events = [
    ...
    @pytest.mark.integration
    def test_spool_persistence_and_cleanup(self, tmp_spool_dir: str) -> None:
        spool_mgr = SpoolManager(spool_dir=tmp_spool_dir)
    ...
    @pytest.mark.integration
    def test_multiple_spool_files_concurrent_access(self, tmp_spool_dir: str) -> None:
        spool_mgr = SpoolManager(spool_dir=tmp_spool_dir)

        for i in range(20):
            spool_mgr.write_ndjson('{"idx": ' + str(i) + "}\n", batch_id=f"batch-{i:02d}")
