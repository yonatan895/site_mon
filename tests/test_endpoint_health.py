...
class TestInit:
    def test_initial_statuses_created(self, test_endpoints: list[SourceEndpoint]) -> None:
    ...
    def test_thread_not_started_by_default(self, test_endpoints: list[SourceEndpoint]) -> None:
        checker = EndpointHealthChecker(test_endpoints)
        assert checker._thread is None


class TestStartStop:
    def test_start_creates_thread(self, checker: EndpointHealthChecker) -> None:
        checker.start()
        assert checker._thread is not None
        assert checker._thread.is_alive()
        checker.stop()

    def test_double_start_no_error(self, checker: EndpointHealthChecker) -> None:
        checker.start()
        checker.start()
        assert checker._thread is not None
        checker.stop()

    def test_stop_joins_thread(self, checker: EndpointHealthChecker) -> None:
        checker.start()
        assert checker._thread is not None
        checker.stop()
    ...
    def test_stop_without_start(self, checker: EndpointHealthChecker) -> None:
        checker.stop()
...
class TestGetStatuses:
    def test_get_all_statuses(self, test_endpoints: list[SourceEndpoint]) -> None:
        checker = EndpointHealthChecker(endpoints=test_endpoints, check_interval=1)
    ...
    def test_get_status_unknown_endpoint(self, checker: EndpointHealthChecker) -> None:
