...
class TestEventsToHECLines:
    def test_single_event(self) -> None:
        ...
        event = PollingEvent(platform="hmc", site="primary", data_type="cpc-stats", sourcetype="hmc:cpc_stats", index="mainframe_metrics", fields={"cpc_name": "CPCA"})
    ...
    def test_list_of_events(self) -> None:
        ...
        poller = PollerCls.__new__(PollerCls)
        endpoint = SourceEndpoint(name="test-ep", url="https://test.example.com", platform="hmc", site="primary", auth_type="basic", creds_vault_path="secret/test")
        events = [PollingEvent(platform="hmc", site="primary", data_type="cpc-stats", sourcetype="hmc:cpc_stats", index="mainframe_metrics", fields={"cpc_name": "CPA"}), PollingEvent(platform="hmc", site="primary", data_type="cpc-stats", sourcetype="hmc:cpc_stats", index="mainframe_metrics", fields={"cpc_name": "CPB"})]
...
class TestPollingEventToHEC:
    def test_correct_format(self) -> None:
        ...
        poller = PollerCls.__new__(PollerCls)
        endpoint = SourceEndpoint(name="test-ep", url="https://test.example.com", platform="hmc", site="primary", auth_type="basic", creds_vault_path="secret/test")
        event = PollingEvent(platform="hmc", site="primary", data_type="cpc-stats", sourcetype="hmc:cpc_stats", index="mainframe_metrics")
...
class TestCreateClient:
    ...
    def test_unsupported_platform_raises(self) -> None:
        from src.poller import Poller as PollerCls
        poller = PollerCls.__new__(PollerCls)
        endpoint = SourceEndpoint(name="test-ep", url="https://test.example.com", platform="unknown", site="main", auth_type="basic", creds_vault_path="secret/test")
...
class TestHMCClient:
    ...
    def hmc_endpoint(self) -> SourceEndpoint:
        return SourceEndpoint(name="hmc-test", url="https://hmc.example.com", platform="hmc", site="primary", auth_type="basic", creds_vault_path="secret/test")
    ...
    def test_connect(self, hmc_endpoint: SourceEndpoint, mock_session: MagicMock) -> None:
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "admin", "HMC_PRIMARY_PASSWORD": "pass"}):
            mock_zhmc = MagicMock()
    ...
    def test_connect_reuses_session(self, hmc_endpoint: SourceEndpoint, mock_session: MagicMock) -> None:
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "admin", "HMC_PRIMARY_PASSWORD": "pass"}):
            mock_zhmc = MagicMock()
            mock_zhmc.Session.return_value = mock_session
            with patch.dict(sys.modules, {"zhmcclient": mock_zhmc}):
    ...
    def test_load_creds(self, hmc_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "myuser", "HMC_PRIMARY_PASSWORD": "mypass"}):
            from src.poller import HMCClient
            client = HMCClient(hmc_endpoint)
    ...
    def test_query_cpc_stats(self, hmc_endpoint: SourceEndpoint, mock_session: MagicMock) -> None:
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "admin", "HMC_PRIMARY_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"zhmcclient": mock_zhmc}):
                from src.poller import HMCClient
                client = HMCClient(hmc_endpoint)
                result = client.query("cpc-stats", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
    ...
    def test_query_lpars(self, hmc_endpoint: SourceEndpoint, mock_session: MagicMock) -> None:
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "admin", "HMC_PRIMARY_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"zhmcclient": mock_zhmc}):
                from src.poller import HMCClient
                client = HMCClient(hmc_endpoint)
                result = client.query("lpars", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
    ...
    def test_query_chpids(self, hmc_endpoint: SourceEndpoint, mock_session: MagicMock) -> None:
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "admin", "HMC_PRIMARY_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"zhmcclient": mock_zhmc}):
                from src.poller import HMCClient
                client = HMCClient(hmc_endpoint)
                result = client.query("chpid", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
                assert result[0]["adapter_id"] == "CHP01"

    def test_query_unknown_type(self, hmc_endpoint: SourceEndpoint, mock_session: MagicMock) -> None:
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "admin", "HMC_PRIMARY_PASSWORD": "pass"}):
            mock_zhmc = MagicMock()
            mock_zhmc.Session.return_value = mock_session
            with patch.dict(sys.modules, {"zhmcclient": mock_zhmc}):
                from src.poller import HMCClient
                client = HMCClient(hmc_endpoint)
                result = client.query("unknown", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert result == []
...
class TestDS8000Client:
    ...
    def ds_endpoint(self) -> SourceEndpoint:
        return SourceEndpoint(name="ds-test", url="https://ds.example.com", platform="ds", site="main", auth_type="basic", creds_vault_path="secret/test")
    ...
    def test_connect(self, ds_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"DS_MAIN_USERNAME": "admin", "DS_MAIN_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"pyds8k": mock_pyds8k, "pyds8k.client": mock_pyds8k.client}):
    ...
    def test_query_arrays(self, ds_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"DS_MAIN_USERNAME": "admin", "DS_MAIN_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"pyds8k": mock_pyds8k, "pyds8k.client": mock_pyds8k.client}):
                from src.poller import DS8000Client
                client = DS8000Client(ds_endpoint)
                result = client.query("arrays", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
    ...
    def test_query_ports(self, ds_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"DS_MAIN_USERNAME": "admin", "DS_MAIN_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"pyds8k": mock_pyds8k, "pyds8k.client": mock_pyds8k.client}):
                from src.poller import DS8000Client
                client = DS8000Client(ds_endpoint)
                result = client.query("ports", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
    ...
    def test_query_ranks(self, ds_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"DS_MAIN_USERNAME": "admin", "DS_MAIN_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"pyds8k": mock_pyds8k, "pyds8k.client": mock_pyds8k.client}):
                from src.poller import DS8000Client
                client = DS8000Client(ds_endpoint)
                result = client.query("ranks", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
    ...
    def test_query_replication(self, ds_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"DS_MAIN_USERNAME": "admin", "DS_MAIN_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"pyds8k": mock_pyds8k, "pyds8k.client": mock_pyds8k.client}):
                from src.poller import DS8000Client
                client = DS8000Client(ds_endpoint)
                result = client.query("replication", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
...
class TestCSMClient:
    ...
    def csm_endpoint(self) -> SourceEndpoint:
        return SourceEndpoint(name="csm-test", url="https://csm.example.com", platform="csm", site="main", auth_type="basic", creds_vault_path="secret/test")
    ...
    def test_query_sessions(self, csm_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"CSM_MAIN_USERNAME": "admin", "CSM_MAIN_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"pycsm": mock_pycsm}):
                from src.poller import CSMClient
                client = CSMClient(csm_endpoint)
                result = client.query("sessions", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
    ...
    def test_query_policies(self, csm_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"CSM_MAIN_USERNAME": "admin", "CSM_MAIN_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"pycsm": mock_pycsm}):
                from src.poller import CSMClient
                client = CSMClient(csm_endpoint)
                result = client.query("policies", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
                assert len(result) == 1
...
class TestTS7700Client:
    ...
    def ts_endpoint(self) -> SourceEndpoint:
        return SourceEndpoint(name="ts-test", url="https://ts.example.com", platform="ts7700", site="primary", auth_type="basic", creds_vault_path="secret/test")

    def test_get_session(self, ts_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"TS7700_PRIMARY_USERNAME": "admin", "TS7700_PRIMARY_PASSWORD": "pass"}):
    ...
    def test_query_cluster(self, ts_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"TS7700_PRIMARY_USERNAME": "admin", "TS7700_PRIMARY_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"requests": mock_requests}):
                from src.poller import TS7700Client
                client = TS7700Client(ts_endpoint)
                result = client.query("cluster", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
    ...
    def test_query_cache(self, ts_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"TS7700_PRIMARY_USERNAME": "admin", "TS7700_PRIMARY_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"requests": mock_requests}):
                from src.poller import TS7700Client
                client = TS7700Client(ts_endpoint)
                result = client.query("cache", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
    ...
    def test_query_drives(self, ts_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"TS7700_PRIMARY_USERNAME": "admin", "TS7700_PRIMARY_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"requests": mock_requests}):
                from src.poller import TS7700Client
                client = TS7700Client(ts_endpoint)
                result = client.query("drives", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
    ...
    def test_load_creds(self, ts_endpoint: SourceEndpoint) -> None:
        with patch.dict(os.environ, {"TS7700_PRIMARY_USERNAME": "u", "TS7700_PRIMARY_PASSWORD": "p"}):
            from src.poller import TS7700Client
            client = TS7700Client(ts_endpoint)
            creds = client._load_creds()
        ...
        with patch.dict(os.environ, {"HMC_PRIMARY_USERNAME": "admin", "HMC_PRIMARY_PASSWORD": "pass"}):
            ...
            with patch.dict(sys.modules, {"zhmcclient": mock_zhmc}):
                ...
                p.source_selector.get_active_endpoints = MagicMock(return_value=[
                    SourceEndpoint(name="hmc-primary", url="https://test.example.com", platform="hmc", site="primary", auth_type="basic", creds_vault_path="secret/test")
                ])
                p.site_configs = {
                    "primary": SiteConfig(site_name="primary", platform="hmc", endpoints=[SourceEndpoint(name="hmc-primary", url="https://test.example.com", platform="hmc", site="primary", auth_type="basic", creds_vault_path="secret/test")], data_types=["cpc-stats"])
                }
                p.platform_rules = {
                    "cpc-stats": PlatformRule(name="cpc-stats", data_type="cpc-stats", sourcetype="hmc:cpc", index="mainframe_metrics")
                }
...
class TestBaseAPIClient:
    def test_query_not_implemented(self) -> None:
        ...
        with pytest.raises(NotImplementedError):
            client.query("any", PlatformRule(name="test", data_type="test", sourcetype="test", index="test"))
